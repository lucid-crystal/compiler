...
describe LC::Parser do
  context "magic vars" do
    it "parses magic line expressions" do
      assign = parse(<<-EXPR).should be_a LC::Assign
    ...
    it "parses magic file expressions" do
      assign = parse(<<-EXPR, file: "my_file.cr").should be_a LC::Assign
      a = __FILE__
      EXPR

      str = assign.value.should be_a LC::StringLiteral
      str.value.should eq "my_file.cr"
    end

    it "parses magic dir expressions" do
      assign = parse(<<-EXPR, dir: "my_dir").should be_a LC::Assign
      a = __DIR__
      EXPR
